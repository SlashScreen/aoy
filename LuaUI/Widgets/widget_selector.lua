...
function widget:DrawScreen()
	...
	gl.Color(0, 0, 0, 0.8)
	RectRound(
		minx - (bgPadding * sizeMultiplier),
		miny - (bgPadding * sizeMultiplier),
		maxx + (bgPadding * sizeMultiplier),
		maxy + (bgPadding * sizeMultiplier),
		8 * sizeMultiplier
	)
...
function widget:MousePress(x, y, button)
	...
	if button == 1 then
		...
		if
			maxx - 10 < x
			and x < maxx
			and maxy + bgPadding < y
			and y < maxy + ((buttonFontSize + 7 + bgPadding) * sizeMultiplier)
		then
			return true
		end
		if
...
function widget:MouseMove(x, y, dx, dy, button)
	if show and activescrollbar then
		startEntry = math.max(
			...
			math.min(
				math.floor(
					(#fullWidgetsList * ((sby1 - sbsize) - (y - math.min(scrollbargrabpos, sbsize))) / sbheight) + 0.5
				),
				#fullWidgetsList - curMaxEntries
			)
		) + 1
...
function widget:IsAbove(x, y)
	...
	if
		(x < minx)
		or (x > maxx + (yStep * sizeMultiplier))
		or (y < miny - #buttons * buttonHeight)
		or (y > maxy + bgPadding)
	then
		return false
	end
