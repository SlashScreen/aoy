...
function widget:DrawScreen()
  ...
  -- draw the box
  gl.Color(0, 0, 0, 0.8)
  RectRound(minx - (bgPadding * sizeMultiplier), miny - (bgPadding * sizeMultiplier), maxx + (bgPadding * sizeMultiplier),
    maxy + (bgPadding * sizeMultiplier), 8 * sizeMultiplier)
  ...
  for i, name in ipairs(buttons) do
    ...
    gl.Text(tcol .. buttons[i], (minx + maxx) / 2, miny - (buttonTop * sizeMultiplier) -
    (i * (buttonHeight * sizeMultiplier)), buttonFontSize * sizeMultiplier, "oc")
  end
  ...
  if #widgetsList < #fullWidgetsList then
    ...
    if (sbposx < mx and mx < sbposx + sbsizex and sby2 < my and my < sby2 + sbheight) then
      ...
      RectRound(sbposx + scrollerPadding, sbposy, sbposx + sbsizex - scrollerPadding, sbposy + sbsizey,
        1.75 * sizeMultiplier)
      gl.Blending(GL.SRC_ALPHA, GL.ONE_MINUS_SRC_ALPHA)
    end
    gl.Color(0.33, 0.33, 0.33, 0.8)
    RectRound(sbposx + scrollerPadding, sbposy, sbposx + sbsizex - scrollerPadding, sbposy + sbsizey, 1.75 *
    sizeMultiplier)
  else
    sbposx = 0.0
    sbposy = 0.0
...
function widget:MouseMove(x, y, dx, dy, button)
  if show and activescrollbar then
    startEntry = math.max(0,
      math.min(
        math.floor((#fullWidgetsList * ((sby1 - sbsize) - (y - math.min(scrollbargrabpos, sbsize))) / sbheight) + 0.5),
        #fullWidgetsList - curMaxEntries)) + 1
    UpdateListScroll()
    return true
  end
