...
--- @field UnloadAllDocuments fun(self: RmlContext):nil Closes all documents currently loaded with the context.
--- @field UnloadDocument fun(self: RmlContext, document: RmlDocument):nil Unloads a specific document within the context.
--- @field Update fun(self: RmlContext):boolean Updates the context.

--- @class RmlContextDocumentsProxy:nil
...
--- @field title string
--- @field Close fun(self: RmlDocument):nil Hides and closes the document, destroying its contents.
...
--- @field Hide fun(self: RmlDocument):nil Hides the document.
--- @field PullToFront fun(self: RmlDocument):nil Pulls the document in front of other documents within its context with a similar z-index.
--- @field PushToBack fun(self: RmlDocument):nil Pushes the document behind other documents within its context with a similar z-index.
--- @field Show fun(self: RmlDocument, modal: RmlDocumentModal?, focus: RmlDocumentFocus?):nil Shows the document. flags is either NONE, FOCUS or MODAL. flags defaults to FOCUS.
...
--- @field AddEventListener fun(self: RmlElement, event: boolean, listener: function|string, in_capture_phase: boolean):nil NOTE: Events added from python cannot be removed.
--- @field AppendChild fun(self: RmlElement, element: RmlElementPtr):nil Appends element as a child to this element.
--- @field Blur fun(self: RmlElement):nil Removes input focus from this element.
--- @field Click fun(self: RmlElement):nil Fakes a click on this element.
--- @field DispatchEvent fun(self: RmlElement, event: string, parameters: table, interruptible: string):nil Dispatches an event to this element. The event is of type event. Parameters to the event are given in the dictionary parameters; the dictionary must only contain string keys and floating-point, integer or string values. interruptible determines if the event can be forced to stop propagation early.
--- @field Focus fun(self: RmlElement):nil Gives input focus to this element.
...
--- @field InsertBefore fun(self: RmlElement, element: RmlElementPtr, adjacent_element: RmlElement):nil Inserts the element element as a child of this element, directly before adjacent_element in the list of children.
...
--- @field RemoveAttribute fun(self: RmlElement, name: string):nil Removes the attribute named name from the element.
...
--- @field ScrollIntoView fun(self: RmlElement, align_with_top: boolean):nil Scrolls this element into view if its ancestors have hidden overflow. If align_with_top is True, the element's top edge will be aligned with the top (or as close as possible to the top) of its ancestors' viewing windows. If False, its bottom edge will be aligned to the bottom.
--- @field SetAttribute fun(self: RmlElement, name: string, value: string):nil Sets the value of the attribute named name to value.
--- @field SetClass fun(self: RmlElement, name: string, value: boolean):nil Sets (if value is true) or clears (if value is false) the class name on the element.
...
--- @field AddColumn fun(self: RmlElementDataGrid, fields: string, formatter: string, initial_width: number, header_rml: string):nil Adds a new column to the data grid. The column will read the columns fields (in CSV format) from the grid's data source, processing it through the data formatter named formatter. header_rml specifies the RML content of the column's header.
--- @field SetDataSource fun(self: RmlElementDataGrid, data_source_name: string):nil Sets the name and table of the new data source to be used by the data grid.
...
--- @field Submit fun(self: RmlElementForm, submit_value: string?, submit_value: string?):nil Submits the form with a submit value of submit_value.
...
--- @field Remove fun(self: RmlElementFormControlSelect, index: integer):nil Removes an existing option from the selection box.
--- @field RemoveAll fun(self: RmlElementFormControlSelect):nil
...
--- @field Remove fun(self: RmlElementFormControlTextArea, index: integer):nil Removes an existing option from the selection box.
--- @field RemoveAll fun(self: RmlElementFormControlTextArea):nil

--- @class RmlElementInstancer:nil
---
--- @field new fun():RmlElementInstancer
--- @field InstanceElement fun(self: RmlElementInstancer):any

--- @class RmlElementPtr:nil
...
--- @field SetPanel fun(self: RmlElementTabSet, index: integer, rml: string):nil Sets the contents of a panel to the RML content rml. If index is out-of-bounds, a new panel will be added at the end.
--- @field SetTab fun(self: RmlElementTabSet, index: integer, rml: string):nil Sets the contents of a tab to the RML content rml. If index is out-of-bounds, a new tab will be added at the end.
...
--- @field type string
--- @field StopPropagation fun(self: RmlEvent):nil Stops the propagation of the event through the event cycle, if allowed.
...
--- @class RmlGlobalLuaFunctions:nil
--- @see RmlUi
...
--- @field LogMessage fun(self: RmlLog, type: RmlLogType, str: string):nil
