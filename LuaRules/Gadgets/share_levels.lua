...
local help = " <on|off|list|toggle|<<e|m|*> [<on|off|%>]>]>" .. ":  control automatic team resource sharing"
...
local function PrintState()
	if not metalForce then
		...
		Spring.Echo(string.format("forced metal sharing level set to: %i%%", ((1 - metalLevel) * 100)))
	end
	if not energyForce then
		Spring.Echo("forced energy sharing is disabled")
	else
		Spring.Echo(string.format("forced energy sharing level set to: %i%%", ((1 - energyLevel) * 100)))
	end
	return true
end
...
local function ChatControl(cmd, line, words, playerID)
	...
	if not (w1 == "e") and not (w1 == "m") and not (w1 == "*") then
		return PrintHelp()
	end
...
function gadget:GameFrame()
	...
	for _, team in ipairs(GetTeamList()) do
		if metalForce then
			SetTeamShareLevel(team, "m", metalLevel)
		end
		if energyForce then
