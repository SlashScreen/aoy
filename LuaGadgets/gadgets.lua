...
GADGETS_INCLUDE = GADGETS_INCLUDE or {}
local SAFEWRAP = 0
-- 0: disabled
...
local function ArrayConcat(array1, array2)
	local length = #array1

	for i, v in ipairs(array2) do
		table.insert(array1, length + i, v)
	end
end
...
function gadgetHandler:Initialize()
	...
	ArrayConcat(gadgetFiles, GADGETS_INCLUDE)
...
function gadgetHandler:GotChatMsg(msg, player)
	if (player == 0) and Spring.IsCheatingEnabled() then
		...
		local _, _, match = string.find(msg, sp .. "togglegadget" .. ep)
		if match then
			self:ToggleGadget(match)
			return true
		end
		_, _, match = string.find(msg, sp .. "enablegadget" .. ep)
		if match then
			self:EnableGadget(match)
			return true
		end
		_, _, match = string.find(msg, sp .. "disablegadget" .. ep)
		if match then
...
function gadgetHandler:DefaultCommand(type, id, cmd)
	for _, g in r_ipairs(self.DefaultCommandList) do
		local defaultCmdId = g:DefaultCommand(type, id, cmd)
		if defaultCmdId then
			return defaultCmdId
		end
	end
end
