...
do
	...
	local function lex_string_escape(input, i, c)
		...
		elseif lex_decimals[c] then
			local len = lex_decimals[input:sub(i + 1, i + 1)] and (lex_decimals[input:sub(i + 2, i + 2)] and 2 or 1)
				or 0
...
do
	...
	local function fail(ps, i, msg)
		if not ps.tokens[i] then
			...
			table.insert(
				ps.errs,
				{ filename = ps.filename, y = eof.y, x = eof.x, msg = msg or "unexpected end of file" }
			)
			return #ps.tokens
		end
				...
				elseif ps.tokens[i].tk == "=" then
					...
					elseif next_word == "functiontype" then
						...
						return fail(
							ps,
							i,
							"syntax error: this syntax is no longer valid; use 'type " .. v.tk .. " = '..."
						)
					end
				else
...
local function require_module(w, module_name, opts, env)
	...
	if found and (opts.feat_lax == "on" or found:match("tl$")) then
		...
		local defaults = {
			...
			run_internal_compiler_checks = opts.run_internal_compiler_checks
				or save_defaults.run_internal_compiler_checks,
		}
...
do
			...
			if t1.typevals and t2.typevals and #t1.typevals == #t2.typevals then
				...
				for i = 1, #t1.typevals do
					...
					self.errs:add_prefixing(
						nil,
						typeval_errs,
						"type parameter <" .. show_type(t2.typevals[i]) .. ">: ",
						errs
					)
				end
		...
		["function"] = {
			["function"] = function(self, a, b)
				...
				if narets ~= nbrets then
					return false,
						{ Err("different number of return values: got " .. narets .. ", expected " .. nbrets) }
				end
				local errs = {}
				for i = 1, naargs do
				...
				for i = 1, math.min(#a.types, #b.types) do
					if not self:is_a(a.types[i], b.types[i]) then
						return false,
							{
								Err(
									"in tuple entry " .. tostring(i) .. ": got %s, expected %s",
									a.types[i],
									b.types[i]
								),
							}
			...
			["function"] = function(self, a, b)
				...
				if #ar < #br and not diff_by_va then
					table.insert(
						errs,
						Err(
							"incompatible number of returns: got " .. #ar .. " %s, expected " .. #br .. " %s",
							a.rets,
							b.rets
						)
					)
	...
	do
					...
					return nil,
						{
							Err_at(
								w,
								"wrong number of arguments (given " .. given .. ", expects " .. show_arity(f) .. ")"
							),
						}
				end
			...
			check_poly_call = function(self, w, wargs, p, args, expected_rets, cm, argdelta, or_args, or_rets)
				...
				for pass = 1, 3 do
					for i, f in self:iterate_poly(p) do
						...
						then
							local ok, errs =
								check_call(self, w, wargs, f, args, expected_rets, cm, argdelta, or_args, or_rets)
							if ok then
		...
		function TypeChecker:type_check_function_call(node, func, args, argdelta, or_args, or_rets, e1, e2)
			...
			if func.typename == "poly" then
				f, ret, errs =
					check_poly_call(self, node, e2, func, args, expected_rets, cm, argdelta, or_args, or_rets)
	...
	function TypeChecker:type_check_index(anode, bnode, a, b)
		...
		if ra.typename == "tupletable" and rb.typename == "integer" then
			if bnode.constnum then
				if
					bnode.constnum >= 1
					and bnode.constnum <= #ra.types
					and bnode.constnum == math.floor(bnode.constnum)
				then
	...
	local function parse_pattern_string(node, pat, inclempty)
		...
		if unclosed ~= 0 then
			return nil,
				"malformed pattern: " .. unclosed .. " capture" .. (unclosed == 1 and "" or "s") .. " not closed"
		end
		return results
	end
	...
	local special_functions = {
		...
		["ipairs"] = function(self, node, a, b, argdelta)
			...
			if t.typename == "tupletable" then
				...
				if not arr_type then
					return self.errs:invalid_at(
						node.e2,
						"attempting ipairs on tuple that's not a valid array: %s",
						orig_t
					)
				end
			elseif not t.elements then
				if not (self.feat_lax and (is_unknown(t) or t.typename == "emptytable")) then
					return self.errs:invalid_at(
						node.e2,
						"attempting ipairs on something that's not an array: %s",
						orig_t
					)
				end
			end
		...
		["string.pack"] = function(self, node, a, b, argdelta)
			...
			if packstr.typename == "string" and packstr.literal and a.typename == "function" then
				...
				return (
					self:type_check_function_call(node, a, b, argdelta, a_type(node, "tuple", { tuple = items }), nil)
				)
			else
				return (self:type_check_function_call(node, a, b, argdelta))
			end
		...
		["string.unpack"] = function(self, node, a, b, argdelta)
			if #b.tuple < 2 or #b.tuple > 3 then
				return self.errs:invalid_at(
					node,
					"wrong number of arguments (given " .. #b.tuple .. ", expects 2 or 3)"
				)
			end
		...
		["string.gsub"] = function(self, node, a, b, argdelta)
			if #b.tuple < 3 or #b.tuple > 4 then
				return self.errs:invalid_at(
					node,
					"wrong number of arguments (given " .. #b.tuple .. ", expects 3 or 4)"
				)
			end
			local pat = b.tuple[2]
		...
		["forin"] = {
			...
			before_statements = function(self, node, children)
				...
				if exp1type.typename == "poly" then
					local _r, f
					_r, f = self:type_check_function_call(
						exp1,
						exp1type,
						args,
						0,
			...
			after = function(self, node, children)
				...
				for i, child in ipairs(children) do
					...
					elseif decltype.typename == "tupletable" and is_numeric_type(cktype) then
						...
						elseif not dt then
							self.errs:add_in_context(
								node[i],
								node,
								"unexpected index " .. n .. " in tuple %s",
								decltype
							)
						else
						...
						if ambiguous and not t then
							if TL_DEBUG then
								self.errs:add_warning(
									"debug",
									node,
									"the resulting type is ambiguous: %s or %s",
									ua,
									ub
								)
